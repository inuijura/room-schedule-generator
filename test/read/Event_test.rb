...
LOG_DIR = File.join(READ_DIR, 'Event_test_log')

FileUtils.mkdir_p(LOG_DIR)
timestamp = Time.now.strftime('%Y%m%d_%H%M%S')
log_path = File.join(LOG_DIR, "Event_test_#{timestamp}.log")
log_lines = []
...
write_log_line(log_lines, "Event テストログ開始: #{Time.now.iso8601}")
...
# 指示の Event_date を優先し、存在しなければ既存の Event_data を使う
EVENT_SOURCE_DIR = begin
  preferred = File.join(READ_DIR, 'Event_date')
  fallback = File.join(READ_DIR, 'Event_data')
  File.directory?(preferred) ? preferred : fallback
end

unless File.directory?(EVENT_SOURCE_DIR)
  write_log_line(log_lines, "対象ディレクトリが見つかりません")
  File.write(log_path, log_lines.join("\n") + "\n")
  exit 1
end

excel_files = Dir.glob(File.join(EVENT_SOURCE_DIR, '**', '*'))
...
write_log_line(log_lines, "Event テスト開始: #{File.basename(EVENT_SOURCE_DIR)}")
...
excel_files.each do |file_path|
  ...
  begin
    parsed_rows = Event.parse_excel_rows(file_path)
    events = Event.from_parsed_rows(parsed_rows)
    completed = true
